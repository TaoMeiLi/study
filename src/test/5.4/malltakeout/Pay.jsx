...
const
	_k = (key, ...args)=>appUtils.i18n.getKey.apply(null, [$i18n.malltakeout_confirm[key], ...args]),
...
export default React.createClass({
	_baseVars() {
		let
			{data} = this.state,
			{totalPrice, deduct, couponDeductMoney, allCoupons, couponShow, couponSelectedUid, couponSelectedData, usePriorityLogic} = data,
			...
			data.couponDeductMoney = couponDeductMoney ? couponDeductMoney :0;
			console.log(1,this.state);
		return {data, totalPrice, deduct, couponDeductMoney, allCoupons, couponShow, couponSelectedUid, couponSelectedData, usePriorityLogic, hasDeduct};
	},
	_getDeduct(id) {
	...
	_payVars() {
		let
			{data, totalPrice, deduct, couponDeductMoney, allCoupons, couponShow, couponSelectedUid, couponSelectedData, usePriorityLogic, hasDeduct} = this._baseVars(),
			...
			checkedTotal = checkedItems.reduce( (prev,d)=>prev+Math.min(d.onhand.amount, oldVlu(d)), 0 ) + couponDeductMoney,
	...
	_calcPayPart() {
		...
		if (_.isUndefined(this.state.data.deduct)) {
			if(_.isUndefined(this.state.data.couponShow)){
				...
				let moneyDifference=this.state.data.totalPrice-this.state.data.couponDeductMoney;
		...
		let {totalPrice, deduct,couponDeductMoney, allCoupons, couponShow, couponSelectedUid, couponSelectedData, usePriorityLogic, hasDeduct} = this._baseVars();
		...
		if (deductIsEnough) {
			...
			let t = totalPrice-couponDeductMoney;
			cacheForEach(checkedItems)(d=>{ let v = min(d)(t,oldVlu(d)); t -= v; if(v<0){v=0} return v; });
	...
	_onSubmit() {
		let
			...
			errNotice = null;
		if (data.totalPrice) {
			let moneyGap = data.totalPrice-data.couponDeductMoney;
			//不支持微信支付
			if (_.isUndefined(data.payMethods)){
				//没有优惠券抵扣
				if(!couponDeductMoney){
			...
			this.props.onSubmit({
				...
				couponMoney: data.couponDeductMoney,
	...
	_onCoupon(id) {
		let {data, totalPrice, deduct, couponDeductMoney, allCoupons, couponShow, couponSelectedUid, couponSelectedData, usePriorityLogic, hasDeduct} = this._baseVars();
		let getCouponUrl = id=>`${_appFacade.ajaxPrefix}/takeout/coupon/`+id
	...
	onConfirm(couponSelectedUid, couponSelectedData, couponDeductMoney, vouchers, gifts) {
		let data = this.state.data;
		this.voucher = vouchers;
		this.gift = gifts;
		data.couponShow = false;
		data.couponDeductMoney= couponDeductMoney;
		this.isOpen = data.password.items !== undefined ? (this.stored && ~data.password.items.indexOf('stored') ||
									this.intergral && ~data.password.items.indexOf('intergral') ||
									this.voucher && ~data.password.items.indexOf('voucher') ||
									this.gift && ~data.password.items.indexOf('gift'))
						: 0;
		this.setState({
			data,
			isOpen: this.isOpen
	...
	_onToggleState(item, status) {
		...
		this.isOpen = data.password.items !== undefined ? (this.stored && ~data.password.items.indexOf('stored') ||
									this.intergral && ~data.password.items.indexOf('intergral') ||
									this.voucher && ~data.password.items.indexOf('voucher') ||
									this.gift && ~data.password.items.indexOf('gift')) : 0;
			...
			{data.totalPrice
				...
				<div>
					<ul className="mod_common_list deduct">
						...
						{data.couponNum
							?
							<li className="noGap couponstyle">
								<a href="javascript:void(0);"  onClick={this._onCoupon.bind(this,data.id)} >
									<div>优惠券</div>
									<p>{data.couponDeductMoney ? '已抵扣' + data.couponDeductMoney + '元' : data.couponNum + '张'+'可用优惠券'}</p>
								</a>
							</li>
							: null
						}
					...
					<div className="notice" dangerouslySetInnerHTML={{__html: data.notice}}></div>
				</div>
				: null
			}
...
let DeductItem = React.createClass({
	_onCheckboxChange(e) {
		let bool = e ? e.currentTarget.checked : false;
		this.setState({
			isOpen: bool
		});
	...
	getInitialState() {
		...
		return {
			data,
			cost,
			isOpen:this.props.data.using ? this.props.data.using : false
		};
	},
	render() {
		...
		return <li className="noGap">
			...
			<input
				...
				/>
			{
				!!this.state.cost && this.state.isOpen
					? _.isUndefined(_d.password)
						? null
						: _d.password.type === 'sms'
							? <SMSPwd
								delay={_d.password.delay}
								maxLength={_d.password.maxLength}
								onSMS={this.props.onSMS}
								onTurnOff={this._onTurnOff.bind(this)}
	...
	componentDidMount() {
		if (!this.state.cost && this.state.isOpen) {
...
let CouponList = React.createClass({
	...
	confirm() {
		...
		this.props.onConfirm(this.state.couponSelectedUid, this.state.couponSelectedData, couponDeductMoney, vouchers, gifts);
	},
	render() {
