...
export default React.createClass({
	...
	_onCoupon(id) {
		...
		appUtils.doGet(getCouponUrl(id)).then(data=>{
			this.setState({
				coupon: data,
				couponShow: true
			});
	...
	render() {
		...
		{this.state.couponShow
			? <CouponList
				...
				min={parseFloat(this.state.totalPrice)}
				...
				?
					<div className="couponstyle">
						<a href="javascript:void(0);"
							...
							<p>优惠券</p>
							<b>{this.state.couponMoney ? '已抵扣' + this.state.couponMoney + '元' : data.couponNum + '张'+'可用优惠券'}</b>
						</a>
					</div>
				: null
			}
