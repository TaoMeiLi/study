...
export default React.createClass({
	_baseVars() {
		...
		return {data, totalPrice, deduct, couponMoney, coupon, couponShow, selected, coupons, usePriorityLogic, hasDeduct};
	},
	_getDeduct(id) {
	...
	_payVars() {
		let
			{data, totalPrice, deduct, couponMoney, coupon, couponShow, selected, coupons, usePriorityLogic, hasDeduct} = this._baseVars(),
	...
	_calcPayPart() {
		if (_.isUndefined(this.state.data.deduct) && _.isUndefined(this.state.data.couponMoney)) {
			this._needToPay = this.state.data.totalPrice;
			return;
		}
		let {totalPrice, deduct, couponMoney, coupon, couponShow, selected, coupons, usePriorityLogic, hasDeduct} = this._baseVars();
		...
		let priceDifference = totalPrice-couponMoney;
		alert(couponMoney);
		if(couponMoney){
			if(priceDifference>0){
				needToPay = priceDifference;
			}else{
				needToPay = 0;
			}
		}else{
	...
	_onSubmit() {
		...
		if (data.totalPrice) {
			...
			if (_.isUndefined(data.payMethods)){
				//如果支持的支付方式都没有打开或者优惠券抵扣金额不够：请选择支付方式；
				if ( !deduct.filter(d=>d.using).length || priceDifference>0) {
					errNotice = '请选择支付方式';
				}
