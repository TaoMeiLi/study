...
export default React.createClass({
	_baseVars() {
		let
			...
			data.coupon = coupon;
		let hasDeduct = !_.isUndefined(deduct) || (_.isArray(deduct) && !_.isEmpty(deduct)),
			hasSelected = !_.isUndefined(coupon.selectedUid) || (_.isArray(coupon.selectedUid) && !_.isEmpty(coupon.selectedUid)),
			hasCoupons = !_.isUndefined(coupon.couponId) || (_.isArray(coupon.couponId) && !_.isEmpty(coupon.couponId)),
			hasAllConpons = !_.isUndefined(coupon.allConpons) || (_.isArray(coupon.allConpons) && !_.isEmpty(coupon.allConpons));
			deduct = hasDeduct ? deduct : [];
			coupon.selectedUid = hasSelected ? coupon.selectedUid : [];
			coupon.couponId = hasCoupons ? coupon.couponId : [];
			coupon.couponMoney = coupon.couponMoney ? coupon.couponMoney :0;
			coupon.allConpons = coupon.allConpons ? coupon.allConpons :[];
			console.log(111,this.state);
		return {data, totalPrice, deduct, coupon, usePriorityLogic, hasDeduct};
	},
	_getDeduct(id) {
	...
	_payVars() {
		let
			{data, totalPrice, deduct, coupon, usePriorityLogic, hasDeduct} = this._baseVars(),
			...
			checkedTotal = checkedItems.reduce( (prev,d)=>prev+Math.min(d.onhand.amount, oldVlu(d)), 0 ) + coupon.couponMoney,
	...
	_calcPayPart() {
		if (_.isUndefined(this.state.data.deduct) && _.isUndefined(this.state.data.coupon.couponShow)) {
			this._needToPay = this.state.data.totalPrice;
			return;
		}
		let {data, totalPrice, deduct, coupon, usePriorityLogic, hasDeduct} = this._baseVars();
	...
	_onSubmit() {
		let
			...
			errNotice = null;
		if (data.totalPrice) {
			let moneyGap = data.totalPrice-data.coupon.couponMoney;
			//不支持微信支付
			...
			this.props.onSubmit({
				...
				couponMoney: data.coupon.couponMoney,
				coupon: data.coupon.selectedUid,
	...
	_onCoupon(id) {
		...
		appUtils.doGet(getCouponUrl(id)).then(item=>{
			...
			data.coupon.allConpons = item;
			data.coupon.couponShow = true;
			this.setState({data});
			console.log(this.state);
		});
	},
	onConfirm (selectedUid,couponId, couponMoney, vouchers, gifts) {
		let data = this.state.data;
		this.voucher = vouchers;
		this.gift = gifts;
		data.coupon.couponShow = false;
		data.coupon.couponMoney= couponMoney;
		this.setState({data});
	},
	...
	render() {
		let {data} = this.state;
		return <div>
		{data.coupon.couponShow
			? <CouponList
				couponData={data.coupon.allConpons}
				selectedUid={data.coupon.selectedUid}
					...
					?
						<div className="couponstyle">
							<a href="javascript:void(0);"
								...
								<p>优惠券</p>
								<b>{data.coupon.couponMoney ? '已抵扣' + data.coupon.couponMoney + '元' : data.couponNum + '张'+'可用优惠券'}</b>
							</a>
						</div>
					: null
				}
...
let CouponList = React.createClass({
	...
	couponSelect(e) {
		...
		if (e.currentTarget.parentNode.className === 'active') {
			index = this.state.selectedUid.indexOf(e.currentTarget.parentNode.id);
			this.state.selectedUid.splice(index, 1);

			this.state.couponId.forEach(function(n, i) {
				if (n.id === e.currentTarget.parentNode.getAttribute('data-coupon')) {
					index = i;
					return false;
				}
			});
			this.state.couponId.splice(index, 1);
			this.setState({
		...
		// 已选择
		this.state.selectedUid.push(e.currentTarget.parentNode.id);
		// 已选择的券的 id 和 share
		this.state.couponId.push({
	...
	confirm() {
		...
		this.state.selectedUid.forEach(item => {
		...
		this.state.couponId.forEach(item => {
		...
		this.props.onConfirm(this.state.selectedUid, this.state.couponId, couponMoney, vouchers, gifts);
	},
	render() {
		var allConpons = this.state.couponData,
			couponId = this.state.couponId ? this.state.couponId : [],
			selectedUid = this.state.selectedUid ? this.state.selectedUid : [],
			disableArr = [];
			return (
			<div className="coupon-list">
				<ul className="avaliable">
					{allConpons.map(item => {
						let count = 0, flag;
						if (this.state.min < item.min && (selectedUid.length === 0 || !~selectedUid.indexOf(item.c2uid))) {
							disableArr.push({
								c2uid: item.c2uid,
								reason: 1
							});
						...
						couponId.forEach(function(n, i) {
							if (n.id !== item.couponId && (n.share && !item.share || !n.share)  && (selectedUid.length === 0 || !~selectedUid.indexOf(item.c2uid))) {
								flag = 1;
								return false;
							}
						...
						if (item.sheets && count >= item.sheets && (selectedUid.length === 0 || !~selectedUid.indexOf(item.c2uid))) {
							disableArr.push({
								c2uid: item.c2uid,
								reason: 2
							});
						...
						return (
							<CouponItem
								active={selectedUid.length && !!~selectedUid.indexOf(item.c2uid) ? 'active' : ''}
								data={item}
