...
export default React.createClass({
	_baseVars() {
		let
			{data} = this.state,
			{totalPrice, deduct, couponMoney, allCoupons, couponShow,selectedData, usePriorityLogic} = data,
			hasDeduct = !_.isUndefined(deduct) || (_.isArray(deduct) && !_.isEmpty(deduct));
			hasSelectedData = !_.isUndefined(selectedData) || (_.isArray(selectedData) && !_.isEmpty(selectedData));
			deduct = hasDeduct ? deduct : [];
			data.selectedData = hasSelectedData ? selectedData : [];
			data.couponMoney = couponMoney ? couponMoney :0;
			console.log(11,this.state);
		return {data, totalPrice, deduct, couponMoney, allCoupons, couponShow, selectedData, usePriorityLogic, hasDeduct,hasSelectedData};
	},
	_getDeduct(id) {
	...
	_payVars() {
		let
			{data, totalPrice, deduct, couponMoney, allCoupons, couponShow, selectedUid, selectedData, usePriorityLogic, hasDeduct, hasSelected, hasCoupons} = this._baseVars(),
	...
	_calcPayPart() {
		...
		let {data, totalPrice, deduct, couponMoney, allCoupons, couponShow, selectedUid, selectedData, usePriorityLogic, hasDeduct, hasSelected, hasCoupons} = this._baseVars();
	...
	onConfirm (selectedUid,selectedData, couponMoney, vouchers, gifts) {
		let data = this.state.data;
...
let CouponList = React.createClass({
	...
	couponSelect(e) {
		...
		if (e.currentTarget.parentNode.className === 'active') {
			...
			this.state.selectedData.forEach(function(n, i) {
	...
	confirm() {
		...
		this.props.onConfirm(this.state.selectedUid, this.state.selectedData, couponMoney, vouchers, gifts);
	},
	render() {
			...
			<div className="coupon-list">
				<ul className="avaliable">
					{allCoupons.map(item => {
						...
						selectedData.forEach(function(n, i) {
